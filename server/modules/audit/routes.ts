...
import { type AuditFilters, AuditService } from "./service";
...
app.post("/", authenticateToken, async (c) => {
  ...
  try {
    ...
    const detailsStr =
      typeof parsedBody.details === "string" ? parsedBody.details : JSON.stringify(parsedBody.details ?? {});

    // Fire-and-forget pattern (audit should not block request)
    try {
      void AuditService.createLog(user.employee_number, parsedBody.action, detailsStr, getClientIp(c));
    } catch (err) {
