...
let useMock = !connectionString || connectionString.includes("REDACTED") || connectionString.includes("placeholder");
...
function isConnectionError(err: any): boolean {
  ...
  const connectionMessages = ["timeout", "no password supplied", "password authentication failed", "database"];
  return (
...
export const db: DatabaseClient = {
  ...
  async transaction<T>(fn: (client: TransactionClient) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (isConnectionError(err)) {
        ...
        return pglite.transaction(async (tx) => {
          const wrappedClient: TransactionClient = {
            async query<T = any>(sql: string, params: any[] = []): Promise<T[]> {
              const { rows } = await tx.query(sql, params); return rows as T[];
            },
            async queryOne<T = any>(sql: string, params: any[] = []): Promise<T | null> {
              const { rows } = await tx.query(sql, params); return (rows[0] as T) || null;
            },
            async execute(sql: string, params: any[] = []): Promise<void> {
              if (params.length > 0) { await tx.query(sql, params); } else { await tx.exec(sql); }
            },
          };
          return fn(wrappedClient);
        });
    ...
    const wrappedClient: TransactionClient = {
      async query<T = any>(sql: string, params: any[] = []): Promise<T[]> {
        const { rows } = await client.query(sql, params); return rows;
      },
      async queryOne<T = any>(sql: string, params: any[] = []): Promise<T | null> {
        const { rows } = await client.query(sql, params); return rows[0] || null;
      },
      async execute(sql: string, params: any[] = []): Promise<void> {
