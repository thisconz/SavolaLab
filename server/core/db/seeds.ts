  ...
  try {
    await db.transaction(async (client: TransactionClient) => {
      ...
      const getLineId = async (name: string): Promise<number> => {
        ...
        if (rows.length === 0)
          throw new Error(`Missing production line: ${name}`);
        return rows[0].id;
      };
      ...
      if (permCount === 0) {
        ...
        for (let i = 0; i < roles.length; i++) {
          await client.execute(
            ...
            [
              roles[i],
              permissions[i].view_results,
              permissions[i].input_data,
              permissions[i].edit_formulas,
              permissions[i].change_specs,
            ],
          );
        }
      }
      ...
      // --- Employees & Admin ---
      const employeesCountResult = await client.query<{ count: string }>(
      ...
      const linesCountResult = await client.query<{ count: string }>(
      ...
      if (linesCount === 0) {
        ...
        await client.execute(
          ...
          ["Centrifuge C-101", await getLineId("Centrifuge"), "Centrifuge"],
        );
        await client.execute(
          "INSERT INTO equipment (name, line_id, type) VALUES ($1, $2, $3)",
          ["Evaporator E-202", await getLineId("Evaporation"), "Evaporator"],
        );
        await client.execute(
          "INSERT INTO equipment (name, line_id, type) VALUES ($1, $2, $3)",
          ["Packaging P-303", await getLineId("Packaging"), "Packer"],
        );
      }

      // --- Shifts ---
      const shiftCountResult = await client.query<{ count: string }>(
      ...
      const sampleCountResult = await client.query<{ count: string }>(
        "SELECT COUNT(*) AS count FROM samples",
      );
      if (Number(sampleCountResult[0].count) === 0) {
        const line = await client.queryOne(
          "SELECT id FROM production_lines WHERE name = $1",
          ["Raw Handling"],
        );
        const equip = await client.queryOne(
          "SELECT id FROM equipment WHERE name = $1",
          ["Centrifuge C-101"],
        );
        const shift = await client.queryOne(
          "SELECT id FROM shifts WHERE name = $1",
          ["Morning"],
        );
      ...
      // --- Sample Types ---
      const typeCountResult = await client.query<{ count: string }>(
        "SELECT COUNT(*) AS count FROM sample_types",
      );
      if (Number(typeCountResult[0].count) === 0) {
        const sampleTypes = [
          ["Raw Sugar", "Unprocessed"],
          ["White Sugar", "Final refined crystals"],
        ]; // Truncated for brevity
        for (const [name, desc] of sampleTypes) {
          await client.execute(
            "INSERT INTO sample_types (name, description) VALUES ($1, $2)",
            [name, desc],
          );
        }
      }
