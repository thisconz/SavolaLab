          ...
          <nav className="flex items-center gap-2">
            ...
            {path.map((segment, i) => (
              <React.Fragment key={`${segment}-${i}`}>
                <ChevronRight
                  className="w-3 h-3 text-(--color-zenthar-steel)"
                  strokeWidth={4}
                />
                <motion.span
...
const TelemetryModule: FC<TelemetryProps> = ({
  icon: Icon,
  label,
  value,
  status,
  ...
  <div className="flex items-center gap-3 py-1 group/tel cursor-help">
    ...
    <div className="flex flex-col">
      <span className="text-[7px] font-black text-(--color-zenthar-text-muted) uppercase tracking-widest leading-none mb-0.5">
        {label}
      </span>
