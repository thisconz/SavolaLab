...
import {
  ShieldCheck, Radio, Zap, Activity, Lock, LucideIcon,
  Fingerprint, RefreshCw, Cpu, Database, Wifi, WifiOff,
} from "lucide-react";
      ...
      <div className="flex flex-col h-full p-6 gap-8 overflow-y-auto no-scrollbar relative z-10">
        ...
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[10px] font-display font-bold text-(--color-zenthar-text-primary) uppercase tracking-[0.3em]">Live_Channel</h3>
              <span className="text-[7px] font-mono text-(--color-zenthar-text-muted) uppercase tracking-widest">SSE_{sseStatus.toUpperCase()}</span>
            </div>
          </div>
          <div className={clsx(
        ...
        <div className="flex flex-col gap-4 flex-1 min-h-0">
          <div className="flex items-center justify-between px-1">
            <span className="text-[10px] font-black text-(--color-zenthar-text-primary) uppercase tracking-[0.3em]">Intercepts</span>
            {unreadCount > 0 && (
        ...
        <div className="flex flex-col gap-6 pt-6 border-t border-(--color-zenthar-steel)">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-(--color-zenthar-text-primary) uppercase tracking-[0.3em]">Diagnostics</span>
            </div>
            <button
            ...
            >
              <RefreshCw size={12} className={clsx("text-(--color-zenthar-text-muted)", isSyncing && "animate-spin")} />
            </button>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  <Database size={12} className={telemetry.dbSync === "ACTIVE" ? "text-emerald-400" : "text-red-400"} />
                  <span className="text-[9px] font-black text-(--color-zenthar-text-muted) uppercase tracking-widest">Database</span>
                </div>
                <span className={clsx(
              ...
              <div className="space-y-4">
                <SegmentedMetric icon={Cpu}      label="CPU_Load"      value={telemetry.cpuLoad}                      progress={parseInt(telemetry.cpuLoad)} />
                <SegmentedMetric icon={Database} label="Memory"        value={telemetry.memory.split(" / ")[0]}       progress={65} />
                <SegmentedMetric icon={Zap}      label="DB_Latency"    value={telemetry.latency}                      progress={20} color="bg-emerald-500" />
              </div>
              ...
              <div className="relative group overflow-hidden rounded-2xl border border-(--color-zenthar-steel) bg-(--color-zenthar-void) p-5">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <p className="text-[7px] font-black text-brand-primary uppercase tracking-[0.4em]">Throughput</p>
                    <motion.h4
                ...
                <div className="grid grid-cols-2 gap-4 pt-3 border-t border-(--color-zenthar-steel) font-mono text-[10px]">
                  <div>
                    <p className="text-[6px] text-(--color-zenthar-text-muted) uppercase font-black">Pending</p>
                    <p className="text-brand-primary font-bold">{telemetry.stats.pending} REQ</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[6px] text-(--color-zenthar-text-muted) uppercase font-black">Error Rate</p>
                    <p className={clsx(
                      "font-black",
  ...
  <div className="group/metric">
    <div className="flex justify-between items-end mb-1.5 px-1">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[8px] font-black text-(--color-zenthar-text-muted) uppercase tracking-widest">{label}</span>
      </div>
      <span className="text-[10px] font-mono text-(--color-zenthar-text-primary) tabular-nums">{value}</span>
    </div>
    <div className="flex gap-1 h-1.5">
      {Array.from({ length: 12 }).map((_, i) => (
        <div
          key={i}
          className={clsx("flex-1 rounded-[1px] transition-all duration-500", (progress / 8.3) > i ? color : "bg-(--color-zenthar-steel)")}
        />
      ))}
    </div>
  </div>
...
const StatusBadge = ({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) => (
  <div className="flex items-center gap-3 p-3 rounded-xl border border-(--color-zenthar-steel) bg-(--color-zenthar-void) hover:bg-(--color-zenthar-steel) transition-all group">
    ...
    <div className="min-w-0">
      <p className="text-[7px] text-(--color-zenthar-text-secondary) font-black uppercase leading-none mb-0.5">{label}</p>
...
const EmptyState = () => (
  <div className="flex flex-col items-center py-16">
    ...
    <span className="text-[8px] font-black text-(--color-zenthar-text-muted) opacity-50 uppercase tracking-[.5em]">All_Clear</span>
  </div>
);
