...
const NavItem = ({ icon: Icon, label, active, onClick, variant = "default" }: any) => (
  <button
    ...
    className={clsx(
      ...
      active ? "text-(--color-zenthar-text-primary)" : "text-(--color-zenthar-text-muted) group-hover:text-(--color-zenthar-text-secondary)"
    )}
  >
    ...
    <div className={clsx(
      ...
      active ? "scale-110 text-brand-primary" : "group-hover:scale-110 group-hover:text-(--color-zenthar-text-primary)"
    )}>
    ...
    <nav className="w-72 h-full flex flex-col border-r border-(--color-zenthar-steel) bg-(--color-zenthar-carbon) z-50 relative">
      ...
      <div className="p-6 mt-auto bg-linear-to-t from-(--color-zenthar-void) to-transparent space-y-4">
        <div className="grid grid-cols-2 gap-2">
           ...
           >
             <Settings size={14} className="text-(--color-zenthar-text-muted) group-hover:text-brand-primary transition-colors mb-1" />
             <span className="text-[7px] font-black text-(--color-zenthar-text-muted) group-hover:text-(--color-zenthar-text-primary) uppercase tracking-widest">Config</span>
           </button>
           <button 
            onClick={() => setActiveTab("archive")}
            className="flex flex-col items-center justify-center p-3 rounded-2xl bg-(--color-zenthar-steel) border border-(--color-zenthar-border) hover:bg-brand-primary/10 transition-colors group"
           >
             <Archive size={14} className="text-(--color-zenthar-text-muted) group-hover:text-brand-primary transition-colors mb-1" />
             <span className="text-[7px] font-black text-(--color-zenthar-text-muted) group-hover:text-(--color-zenthar-text-primary) uppercase tracking-widest">Vault</span>
           </button>
        </div>
