...
export const SampleQueueSkeleton: React.FC<{ count?: number }> = ({
  count = 4,
}) => (
...
export const TableSkeleton: React.FC<TableSkeletonProps> = ({
  ...
  <div className="flex flex-col">
    ...
    {Array.from({ length: rows }).map((_, ri) => (
      <div
        key={ri}
        className="flex gap-4 px-6 py-4 border-b border-(--color-zenthar-steel)/40"
      >
    ...
    <div className="flex items-end gap-2 flex-1">
      {Array.from({ length: 8 }).map((_, i) => (
        <Skeleton
          ...
          style={
            { height: `${25 + Math.random() * 60}%` } as React.CSSProperties
          }
          rounded="md"
        />
      ))}
...
export const LabBenchSkeleton: React.FC = () => (
  <div className="space-y-4 p-2">
    ...
    {[1, 2, 3].map((i) => (
      <div
        key={i}
        className="rounded-2xl border border-(--color-zenthar-steel) bg-(--color-zenthar-carbon)/60 p-5 space-y-4"
      >
      ...
      <div className="col-span-4 space-y-6">
        <div className="bg-white/70 rounded-[2.5rem] border border-white p-8 space-y-3">
          ...
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="flex gap-3 p-3 rounded-xl border border-(--color-zenthar-steel)/40"
            >
