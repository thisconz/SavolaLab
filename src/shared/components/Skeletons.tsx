...
// FIX: removed duplicate `style` prop — JSX only allows one per attribute.
// Merged background gradient directly into the `style` prop.
export const Skeleton: React.FC<SkeletonProps> = ({ className, rounded = "rounded-lg", style }) => (
  <div
    ...
    style={{
      background:
        "linear-gradient(90deg, rgba(8,8,26,0.8) 25%, rgba(13,13,36,0.9) 50%, rgba(8,8,26,0.8) 75%)",
  ...
  <div className="flex flex-col">
    ...
    >
      {Array.from({ length: columns }).map((_, i) => (
        <Skeleton key={i} className="h-2.5 flex-1 rounded-lg" />
      ))}
    </div>
    {Array.from({ length: rows }).map((_, ri) => (
      <div
        key={ri}
        className="flex gap-4 px-5 py-4"
        style={{ borderBottom: "1px solid rgba(100,120,200,0.06)" }}
      >
