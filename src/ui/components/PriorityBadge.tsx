...
export const PriorityBadge: React.FC<PriorityBadgeProps> = ({
  ...
  const config = {
    ...
    HIGH: {
      color: "bg-orange-500/5 text-orange-600 border-orange-500/20 shadow-[0_0_12px_-4px_rgba(249,115,22,0.2)]",
      icon: AlertCircle,
      label: "High Priority",
    ...
    STAT: {
      color: "bg-red-500/10 text-red-600 border-red-600/30 shadow-[0_0_15px_-3px_rgba(220,38,38,0.3)]",
      icon: Zap,
      label: "Critical Stat",
  ...
  return (
    ...
    >
      {/* Internal Glow Layer */}
      <span className={`absolute inset-0 rounded-full blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${current.glow}`} />

      {/* Icon with Kinetic Energy */}
      <div className={`relative z-10 flex items-center justify-center ${current.animation}`}>
