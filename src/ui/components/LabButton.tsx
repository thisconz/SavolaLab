...
export const LabButton: React.FC<LabButtonProps> = memo(({
  ...
  const variants = {
    primary: "bg-brand-primary text-white shadow-[0_0_20px_rgba(var(--brand-primary-rgb),0.3)] hover:shadow-[0_0_40px_rgba(var(--brand-primary-rgb),0.5)]",
    secondary: "bg-white/[0.03] text-zinc-400 border border-white/10 hover:border-white/30 hover:text-white backdrop-blur-md",
    danger: "bg-red-950/20 text-red-500 border border-red-500/30 hover:bg-red-600 hover:text-white shadow-[inset_0_0_15px_rgba(239,68,68,0.1)]",
    ghost: "bg-transparent text-zinc-500 hover:text-brand-primary hover:bg-brand-primary/5",
    laser: "bg-black text-brand-primary border border-brand-primary/50 shadow-[0_0_15px_rgba(var(--brand-primary-rgb),0.2)]",
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="relative tracking-[0.4em]">
          {children}
        </span>
      </motion.div>
      ...
      <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 opacity-0 group-hover/btn:opacity-100 transition-all">
        <span className="text-[6px] text-brand-primary/40 font-mono tracking-widest">BT-PRTCL-09</span>
      </div>
    </motion.button>
  );
});
