...
import { 
  ShieldAlert, RefreshCcw, Terminal, Cpu, Download, 
  Zap, Brain, Binary, AlertTriangle, Activity, 
  History, ServerCrash
} from "lucide-react";
...
/* --- Enhanced Sidebar --- */
const DiagnosticsSidebar = ({ timestamp, faultId, moduleName, attempts }: { 
  timestamp: number | null, 
  faultId: string | null,
  moduleName: string,
  attempts: number
    ...
    <div className="w-80 border-r border-white/5 bg-zenthar-graphite/40 backdrop-blur-3xl p-8 flex flex-col justify-between shadow-[20px_0_40px_rgba(0,0,0,0.3)]">
      <div className="space-y-8">
        ...
        <div className="space-y-5">
          ...
          <StatMetric label="Fault Protocol" value={faultId || "NULL_PTR"} isItalic color="text-rose-500" />
          <StatMetric label="Target Segment" value={moduleName} />
          <StatMetric label="Neural Integrity" value={attempts > 0 ? `DEGRADED (${attempts})` : "SEVERED"} color="text-amber-500" />
      ...
      <div className="bg-black/40 border border-white/10 p-5 rounded-2xl ring-1 ring-white/5">
        <div className="flex justify-between items-center mb-2">
           <span className="text-[9px] text-zinc-500 font-black uppercase tracking-widest">Fault_Timer</span>
           <History size={12} className="text-zinc-700" />
        </div>
        <div className="text-4xl font-black text-white font-mono tracking-tighter italic">
          {(uptime / 1000).toFixed(1)}<span className="text-lg text-rose-500 ml-1">sec</span>
        </div>
      </div>
    </div>
  );
...
const StatMetric = ({ label, value, color = "text-zinc-200", isItalic = false }: { 
  ...
  <div className="group space-y-1">
    ...
    <div className={clsx("text-[10px] font-mono truncate py-1 px-2 bg-white/5 rounded-md border border-white/5", color, isItalic && "italic font-black")}>
      {value}
    </div>
  </div>
);
          ...
          <main className="flex-1 flex flex-col p-20 justify-center relative overflow-hidden">
            ...
            >
              <header className="flex items-start justify-between mb-16">
                ...
                <ServerCrash size={80} className="text-white/3 absolute -right-10 top-10 rotate-12" />
              </header>
              ...
              <div className="grid grid-cols-5 gap-6 mb-12">
                ...
                <div className="col-span-3 space-y-6">
                  <div className="bg-black/60 border border-white/5 rounded-2xl p-6 ring-1 ring-inset ring-white/5 shadow-2xl">
                    <div className="flex items-center gap-2 mb-4 text-zinc-500">
                      <Terminal size={14} />
                      <span className="text-[10px] font-bold uppercase tracking-widest">Stack_Trace_Buffer</span>
                    </div>
                ...
                <div className="col-span-2 bg-zenthar-graphite/20 border border-white/5 rounded-2xl p-6 flex flex-col justify-between">
                  <div className="text-[10px] text-zinc-600 font-black uppercase tracking-[0.2em] mb-4">Matrix_Status</div>
...
const TacticalButton = ({ icon, label, variant = 'default', onClick }: any) => (
  <button 
    ...
    className={clsx(
      ...
      variant === 'active' && "bg-cyan-500/10 border-cyan-500/30 text-cyan-500 hover:bg-cyan-500 hover:text-black",
      variant === 'danger' && "bg-rose-500/10 border-rose-500/30 text-rose-500 hover:bg-rose-500 hover:text-black",
    )}
  >
    {icon}
