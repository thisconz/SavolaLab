...
import { LucideIcon, RefreshCw, AlertCircle, ChevronDown, Maximize2 } from "lucide-react";
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between px-8 py-6 border-b border-brand-sage/5 bg-white/60 backdrop-blur-md">
        <div className="flex items-center gap-5">
          ...
          <div className="relative flex-shrink-0">
            <div className={`absolute inset-0 blur-lg rounded-xl transition-all duration-500 ${loading ? "bg-brand-primary/40 scale-125" : "bg-brand-primary/10 scale-0 group-hover/panel:scale-110"}`} />
            <div className="relative p-3.5 bg-brand-deep rounded-2xl shadow-lg border border-white/10 group-hover/panel:-translate-y-1 transition-transform duration-500">
              {Icon && <Icon className={`w-5 h-5 ${loading ? "text-brand-primary animate-pulse" : "text-white"}`} />}
            </div>
          </div>
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className={`flex-1 overflow-y-auto custom-scrollbar p-8 relative ${contentClassName}`}>
              ...
              <ErrorBoundary name={`Labrix_Module_${title}`}>
                <div className={`transition-all duration-700 ${loading ? "opacity-20 blur-[2px] grayscale" : "opacity-100"}`}>
                  {children}
                </div>
              </ErrorBoundary>
              ...
              {error && (
                ...
                >
                  ...
                  <h4 className="text-[11px] font-black uppercase tracking-widest text-brand-deep mb-2">Protocol Violation</h4>
                  <p className="text-xs font-mono text-brand-sage mb-8 max-w-[280px] italic">{error}</p>
                  <button onClick={onRefresh} className="px-8 py-3 bg-brand-deep text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-brand-primary transition-all shadow-xl shadow-brand-deep/20">
                    Re-Initialize
                  </button>
                </motion.div>
              )}
