...
import {
  ...
  Activity, Save, ThumbsDown, ThumbsUp, ChevronDown,
...
const TestCard: React.FC<TestCardProps> = ({
  test, value, note, error, colourParams, previousResults = [],
  isReviewing, reviewComment,
  onValueChange, onNoteChange, onColourParam,
  onReviewStart, onReviewCancel, onReviewComment, onApprove, onDisapprove,
  inputRef,
}) => {
  ...
  const rangePercent = rule && hasValue
  ...
  return (
    <motion.div
      ...
      className={clsx(
        ...
        isOutOfRange     ? "border-amber-400/40 bg-amber-500/5" :
        hasValue && !error ? "border-emerald-400/20 bg-emerald-500/5" :
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-(--color-zenthar-steel)/40">
        <div className="flex items-center gap-3">
          <div className={clsx(
            ...
            error ? "bg-red-400" : isOutOfRange ? "bg-amber-400" : hasValue ? "bg-emerald-400" : "bg-brand-sage/30"
          )} />
          ...
          {rule?.description && (
            <div className="group relative">
              <Info size={12} className="text-brand-sage/40 hover:text-brand-primary cursor-help transition-colors" />
            ...
            <div className="px-5 py-3 flex gap-4 flex-wrap">
              ...
              {previousResults.slice(0, 5).map((r: any, i: number) => (
                <div key={i} className="flex items-center gap-2 text-[10px] font-mono">
                  ...
                  <span className="text-brand-sage/30 text-[8px]">{new Date(r.performed_at).toLocaleDateString()}</span>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <div className="p-5 grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-4 items-start">
        <div className="flex flex-col gap-2">
          <label className="text-[8px] font-black text-brand-sage uppercase tracking-widest">Measurement</label>
            ...
            <div className="flex flex-col gap-2">
              {(["absorbance", "brix", "cellLength"] as const).map((param) => (
                <input
                  ...
                  placeholder={param === "absorbance" ? "Absorbance" : param === "brix" ? "Brix %" : "Cell Length (cm)"}
                  disabled={!canInput}
            ...
            <div className="relative">
              ...
              {hasValue && (
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  {isOutOfRange ? <AlertTriangle size={14} className="text-amber-400" /> : <CheckCircle2 size={14} className="text-emerald-400" />}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[8px] font-black text-brand-sage uppercase tracking-widest">Range Check</label>
          <div className="h-[50px] flex flex-col justify-center gap-2">
            <div className="text-center">
              ...
              {rule && hasValue && <span className="text-[9px] font-mono text-brand-sage/50 ml-1">{rule.unit}</span>}
            </div>
            {rule && (
              <>
                <div className="flex justify-between text-[8px] font-mono text-brand-sage/40">
                  <span>{rule.min}</span><span>{rule.max}</span>
                </div>
                <div className="relative h-1.5 w-full bg-(--color-zenthar-steel) rounded-full overflow-hidden">
                  ...
                  {rangePercent !== null && (
                    <motion.div
                      ...
                      className={clsx("absolute left-0 h-full rounded-full transition-colors", isOutOfRange ? "bg-amber-400" : "bg-emerald-500")}
                    />
                  )}
                </div>
              </>
        ...
        <div className="flex flex-col gap-2 min-w-[140px]">
          <label className="text-[8px] font-black text-brand-sage uppercase tracking-widest">Notes</label>
          <input
            type="text"
            value={note}
          ...
          {canReview && (
            isReviewing ? (
              <div className="flex flex-col gap-2 mt-1">
                <input
                  autoFocus type="text" value={reviewComment} onChange={(e) => onReviewComment(e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-1">
                  <button onClick={onApprove}    className="flex items-center justify-center gap-1 py-1.5 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-500 hover:text-white border border-emerald-500/30 rounded-lg text-[9px] font-black uppercase transition-all"><ThumbsUp size={10} /> Pass</button>
                  <button onClick={onDisapprove} className="flex items-center justify-center gap-1 py-1.5 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/30 rounded-lg text-[9px] font-black uppercase transition-all"><ThumbsDown size={10} /> Fail</button>
                </div>
                <button onClick={onReviewCancel} className="text-[8px] font-bold text-brand-sage/50 hover:text-brand-sage uppercase text-center transition-colors">Cancel</button>
              </div>
            ) : (
              <button onClick={onReviewStart} className="mt-1 px-3 py-2 bg-brand-primary/10 hover:bg-brand-primary text-brand-primary hover:text-white border border-brand-primary/20 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all">
                Review
              </button>
            )
          )}
...
export const LabBench: React.FC<LabBenchProps> = memo(({ sample, onComplete }) => {
  ...
  const [reviewState, setReviewState] = useState<{ id: number | null; comment: string }>({ id: null, comment: "" });
  ...
  return (
    <LabPanel title="Lab Bench" icon={FlaskConical} loading={loading} actions={
      <button
      ...
      <div className="flex flex-col h-full gap-5">
        <AnimatePresence>
          {showShortcuts && (
            ...
            >
              <div className="flex justify-between gap-4"><span className="text-brand-sage/60">Save analysis</span><kbd className="px-2 py-0.5 bg-(--color-zenthar-steel) rounded text-brand-primary">⌘ + Enter</kbd></div>
              <div className="flex justify-between gap-4"><span className="text-brand-sage/60">Next field</span><kbd className="px-2 py-0.5 bg-(--color-zenthar-steel) rounded text-brand-primary">Tab</kbd></div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-5 bg-linear-to-r from-brand-primary/10 via-brand-primary/5 to-transparent rounded-3xl border border-brand-primary/20">
          ...
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            <span className="text-[9px] font-black text-brand-sage/60 uppercase tracking-widest">{filledCount}/{tests.length} filled</span>
            <div className="w-32 h-1.5 bg-(--color-zenthar-steel) rounded-full overflow-hidden">
              <motion.div
                animate={{ width: `${progress}%` }}
                transition={{ type: "spring", stiffness: 80 }}
                className={clsx("h-full rounded-full", progress === 100 ? "bg-emerald-400" : "bg-brand-primary")}
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
          {tests.map((test, idx) => (
            <TestCard
              ...
              onApprove={() => { handleReview(test.id, "APPROVED", reviewState.comment || undefined); setReviewState({ id: null, comment: "" }); }}
              onDisapprove={() => { handleReview(test.id, "DISAPPROVED", reviewState.comment || undefined); setReviewState({ id: null, comment: "" }); }}
        ...
        <div className="pt-5 border-t border-(--color-zenthar-steel)/40 space-y-3">
          {hasErrors && (
            <div className="flex items-center gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
              ...
              <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest">Fix validation errors before finalising</p>
            </div>
          )}
          <button
            onClick={handleSave}
          ...
          >
            {isSaving ? (
              ...
              <><Save className="w-5 h-5" /> Finalise Analysis <kbd className="text-[8px] opacity-60">⌘+Enter</kbd></>
            )}
          </button>
