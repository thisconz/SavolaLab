...
export const SampleCard: React.FC<SampleCardProps> = memo(
  ({ sample, active, onClick }) => {
    ...
    const stageLabel =
      sample.source_stage ?? sample.sample_type ?? "Standard Sample";
...
const StatusIndicator = ({ status }: { status: SampleStatus }) => {
  const isDone =
    status === SampleStatus.COMPLETED || status === SampleStatus.APPROVED;
  const isTesting =
    status === SampleStatus.VALIDATING || status === SampleStatus.TESTING;
