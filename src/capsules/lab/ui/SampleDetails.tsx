  ...
  ({ sample, onBack, onStartTesting, onUpdate }) => {
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        const changes = Object.keys(editedSample).filter(
          (key) =>
            editedSample[key as keyof Sample] !== sample[key as keyof Sample],
        );
    ...
    return (
      <LabPanel
        ...
        actions={
          <button
            onClick={onBack}
            className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-black uppercase text-brand-sage hover:text-brand-primary bg-brand-mist/50 rounded-lg transition-all border border-brand-sage/10"
          >
      ...
      >
        <div className="flex flex-col h-full space-y-6 overflow-hidden">
          {/* Status Hero Card */}
          <header
            className={`relative p-6 rounded-3xl border transition-all duration-500 shadow-2xl shadow-brand-deep/5 overflow-hidden ${
              isStat
                ? "bg-white border-lab-laser/20"
          ...
          >
            <div className="relative z-10 flex items-start justify-between">
              <div className="flex items-center gap-5">
                <div
                  className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg transition-transform duration-700 ${
                    isStat
                      ? "bg-lab-laser text-white scale-110"
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${isStat ? "bg-lab-laser text-white animate-pulse" : "bg-brand-mist text-brand-sage"}`}
                    >
                      {sample.priority}
                    </span>
                  </div>
              ...
              <div className="text-right">
                <div className="text-[9px] font-black text-brand-sage uppercase tracking-[0.2em] mb-2">
                  Process Phase
                </div>
                <div
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pr-2">
            ...
            <section>
              <div className="flex items-center gap-2 px-1 mb-3">
                ...
                <h3 className="text-[10px] font-black text-brand-deep uppercase tracking-widest">
                  Metadata Grid
                </h3>
              </div>
                  ...
                  {isEditing ? (
                    <div className="col-span-6 grid grid-cols-2 gap-3 animate-in fade-in slide-in-from-bottom-2">
                      ...
                      <div className="p-4 bg-brand-mist/20 border border-brand-primary/20 rounded-2xl">
                        <label className="text-[9px] font-black text-brand-primary uppercase tracking-widest block mb-2">
                          Priority
                        </label>
                        <select
                          className="w-full bg-transparent text-xs font-bold text-brand-deep uppercase"
                    ...
                    <>
                      ...
                      <DetailTile
                        ...
                        icon={TestTube2}
                        label="Requirement"
                        value={`${sample.test_count} Standard Tests`}
                      />
                    </>
                  )}
                </AnimatePresence>
                ...
                >
                  ...
                  <div className="grid gap-2">
                    {testResults.map((result) => (
                      <div
                        key={result.id}
                        className="flex items-center justify-between p-4 bg-linear-to-r from-brand-mist/40 to-transparent border border-brand-sage/10 rounded-2xl group hover:border-brand-primary/30 transition-all"
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center ${result.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-500" : "bg-brand-primary/10 text-brand-primary"}`}
                          >
                          ...
                          <div>
                            <div className="text-[11px] font-black text-brand-deep uppercase">
                              {result.test_type}
                            </div>
                            <div className="text-[8px] font-bold text-brand-sage tracking-widest">
                              {result.status}
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-xl font-black text-brand-deep tabular-nums">
                            {result.calculated_value || "---"}
                          </span>
                          <span className="text-[9px] font-bold text-brand-sage ml-1 uppercase">
                            {result.unit}
                          </span>
                        </div>
                      </div>
...
const DetailTile = ({
  ...
  >
    <div
      className={`flex items-center gap-1.5 text-[8px] font-black uppercase tracking-tighter mb-1 ${highlight ? "text-brand-primary" : "text-brand-sage"}`}
    >
      <Icon size={10} /> {label}
    </div>
    <div className="text-xs font-black text-brand-deep uppercase truncate">
      {value || "N/A"}
    </div>
  </motion.div>
);
