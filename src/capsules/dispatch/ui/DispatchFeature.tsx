      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-8 pb-8">
        <div className="grid grid-cols-4 gap-8 shrink-0">
          <div className="bg-white p-8 rounded-3xl border border-brand-sage/10 shadow-sm flex flex-col gap-3 relative overflow-hidden group">
            ...
            <p className="text-4xl font-black text-brand-deep tracking-tighter relative z-10">{dispatchData.metrics.pending}</p>
          ...
          <div className="bg-white p-8 rounded-3xl border border-brand-sage/10 shadow-sm flex flex-col gap-3 relative overflow-hidden group">
            ...
            <p className="text-4xl font-black text-brand-deep tracking-tighter relative z-10">{dispatchData.metrics.inTransit}</p>
          ...
          <div className="bg-white p-8 rounded-3xl border border-brand-sage/10 shadow-sm flex flex-col gap-3 relative overflow-hidden group">
            ...
            <p className="text-4xl font-black text-brand-deep tracking-tighter relative z-10">{dispatchData.metrics.delayed}</p>
          ...
          <div className="bg-white p-8 rounded-3xl border border-brand-sage/10 shadow-sm flex flex-col gap-3 relative overflow-hidden group">
            ...
            <p className="text-4xl font-black text-brand-deep tracking-tighter relative z-10">{dispatchData.metrics.critical}</p>
            <div className="flex items-center gap-2 text-xs font-bold text-emerald-500 relative z-10">
              ...
              <div className="flex flex-col gap-4 p-4">
                {dispatchData.activeShipments.length === 0 ? (
                  ...
                  dispatchData.activeShipments.map((shipment: any, i: number) => {
                    ...
                    const icon = isTransit ? <Truck className="w-5 h-5 text-emerald-500" /> :
                                 isLoading ? <Package className="w-5 h-5 text-brand-primary" /> :
                                 isDelayed ? <Clock className="w-5 h-5 text-amber-500" /> :
                                 <CheckCircle2 className="w-5 h-5 text-brand-sage" />;

                    const bg = isTransit ? "bg-emerald-500/5" : isLoading ? "bg-brand-primary/5" : isDelayed ? "bg-amber-500/5" : "bg-brand-mist";
                    const border = isTransit ? "border-emerald-500/20" : isLoading ? "border-brand-primary/20" : isDelayed ? "border-amber-500/20" : "border-brand-sage/20";
              ...
              <div className="flex flex-col gap-4 p-4">
                {dispatchData.qcQueue.length === 0 ? (
                  ...
                  dispatchData.qcQueue.map((item: any, i: number) => {
                    ...
                    return (
                    <div key={i} className="p-5 bg-white border border-brand-sage/20 rounded-2xl hover:border-brand-primary/30 hover:shadow-md transition-all duration-300 cursor-pointer group">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-sm font-black text-brand-deep uppercase tracking-wider">{item.batch}</h4>
                        <span className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-[0.2em] border ${
                          isReleased ? 'text-emerald-600 bg-emerald-500/10 border-emerald-500/20' : 'text-amber-600 bg-amber-500/10 border-amber-500/20'
                        }`}>{item.status}</span>
                      </div>
                      <p className="text-xs text-brand-sage font-medium mb-4">Client: {item.client}</p>
                      <div className="w-full bg-brand-mist rounded-full h-2 overflow-hidden">
                        <div className={`h-2 rounded-full transition-all duration-1000 ease-out ${isReleased ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${item.progress}%` }}></div>
                      </div>
                      <p className="text-[9px] text-brand-sage font-mono font-bold uppercase tracking-[0.2em] mt-3 text-right">{isReleased ? 'Ready for Dispatch' : `${item.testsCompleted}/${item.totalTests} Tests Completed`}</p>
                    </div>
                  )})
                )}
            </div>
