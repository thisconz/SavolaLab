...
import {
  Zap, Plus, AlertCircle, Clock, CheckCircle2, Factory,
  ...
  useEffect(() => {
    const unsubCreated = on("STAT_CREATED", (data) => {
      ...
      debounceTimer.current = setTimeout(async () => {
        ...
        setTimeout(() => setNewStatIds((prev) => { const n = new Set(prev); n.delete(data.id); return n; }), 3000);
      }, 400);
    });
    ...
    return () => { unsubCreated(); unsubUpdated(); clearTimeout(debounceTimer.current); };
  }, [on, selectedStat]);
  ...
  return (
    <div className="h-full flex flex-col gap-6 overflow-hidden bg-(--color-zenthar-graphite)/30 p-2 rounded-3xl">
      ...
      <div className="grid grid-cols-3 gap-4 shrink-0">
        {[
          { label: "Active STATs",     value: active,   color: "text-brand-primary",  bg: "bg-brand-primary/10",  Icon: Activity },
          { label: "Critical",         value: critical, color: "text-red-500",        bg: "bg-red-500/10",        Icon: AlertCircle },
          { label: "Avg Response",     value: "12 min", color: "text-emerald-500",    bg: "bg-emerald-500/10",    Icon: Clock },
        ].map(({ label, value, color, bg, Icon }) => (
          <div key={label} className="bg-(--color-zenthar-carbon) p-5 rounded-2xl border border-brand-sage/10 flex items-center gap-4 relative overflow-hidden group hover:shadow-md transition-all">
            <div className={clsx("w-12 h-12 rounded-2xl flex items-center justify-center", bg, color)}>
              <Icon className="w-6 h-6" />
            </div>
            <div>
              <p className="text-[9px] font-black text-brand-sage uppercase tracking-widest mb-0.5">{label}</p>
          ...
          <LabPanel
            ...
            actions={
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => { setShowNewForm(true); setSelectedStat(null); }}
            ...
            <div className="flex flex-col h-full overflow-y-auto custom-scrollbar pr-2 gap-3">
              {stats.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-brand-sage py-10">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest text-(--color-zenthar-text-primary)">No STAT Requests</p>
                </div>
              ) : (
                <AnimatePresence>
                  {stats.map((stat) => (
                    <motion.div
                      ...
                      onClick={() => { setSelectedStat(stat); setShowNewForm(false); }}
                      className={clsx(
                    ...
                    >
                      ...
                      {newStatIds.has(stat.id) && (
                        <span className="absolute top-2 right-2 text-[7px] font-black text-brand-primary uppercase bg-brand-primary/10 px-1.5 py-0.5 rounded">NEW</span>
                      )}
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[9px] font-mono text-brand-sage/60 uppercase">#{String(stat.id).padStart(4, "0")}</span>
                          </div>
                        </div>
                        <span className={clsx("px-2.5 py-1 rounded-full text-[9px] font-black uppercase border", urgencyColor(stat.urgency))}>
                          {stat.urgency ?? "NORMAL"}
                        </span>
                      </div>
                      <p className="text-xs text-(--color-zenthar-text-muted) line-clamp-2 leading-relaxed pl-8">{stat.reason}</p>
                    </motion.div>
                  ))}
                </AnimatePresence>
              )}
            ...
            {showNewForm ? (
              <motion.div key="form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-brand-sage/10">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-(--color-zenthar-text-primary) tracking-tight">New STAT Request</h2>
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-6">
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      <label className="text-[9px] font-black text-brand-sage uppercase tracking-widest block mb-2">Department *</label>
                    ...
                    <div>
                      <label className="text-[9px] font-black text-brand-sage uppercase tracking-widest block mb-2">Urgency</label>
                      <div className="relative">
                  ...
                  <div>
                    <label className="text-[9px] font-black text-brand-sage uppercase tracking-widest block mb-2">Reason *</label>
                    <div className="relative">
                  ...
                  <div className="mt-auto flex gap-3">
                    <button type="button" onClick={() => setShowNewForm(false)} className="px-6 py-3 text-xs font-bold text-brand-sage hover:bg-(--color-zenthar-void) rounded-xl transition-colors">Cancel</button>
                    <button type="submit" disabled={submitting || !formData.department.trim() || !formData.reason.trim()}
              ...
              <motion.div key={`detail-${selectedStat.id}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                <div className="flex items-start justify-between mb-8 pb-6 border-b border-brand-sage/10">
                  <div>
                    <div className="flex items-center gap-3 mb-3">
                      <span className={clsx("px-3 py-1.5 rounded-full text-[9px] font-black uppercase border", urgencyColor(selectedStat.urgency))}>
                        {selectedStat.urgency ?? "NORMAL"}
                      </span>
                      <span className="text-[9px] font-mono text-(--color-zenthar-text-muted)">STAT-{String(selectedStat.id).padStart(4, "0")}</span>
                    </div>
                    <h2 className="text-3xl font-black text-(--color-zenthar-text-primary) tracking-tight uppercase">{selectedStat.department}</h2>
                  </div>
                  <div className="text-right">
                    <p className="text-[9px] font-black text-brand-sage uppercase tracking-widest mb-1">Submitted</p>
                    <p className="text-sm font-mono text-(--color-zenthar-text-primary)">{new Date(selectedStat.created_at).toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="flex-1 grid grid-cols-12 gap-6 overflow-y-auto custom-scrollbar">
                  <div className="col-span-8 space-y-6">
                    <div>
                      ...
                      <div className="bg-(--color-zenthar-void) rounded-2xl p-6 border border-brand-sage/20 relative overflow-hidden">
                        ...
                        <p className="text-sm text-(--color-zenthar-text-primary) leading-relaxed whitespace-pre-wrap pl-2">{selectedStat.reason}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-span-4">
                    <div className="bg-(--color-zenthar-void) rounded-2xl p-5 border border-brand-sage/20">
                      <h3 className="text-[9px] font-black text-brand-sage uppercase tracking-widest mb-4">Status</h3>
                      <div className={clsx("flex flex-col items-center text-center p-5 rounded-xl border",
                        selectedStat.status === "CLOSED" ? "border-emerald-500/30" : selectedStat.status === "IN_PROGRESS" ? "border-amber-500/30" : "border-brand-primary/20")}>
                        <div className="mb-3">{statusIcon(selectedStat.status)}</div>
                        <p className="text-sm font-bold text-(--color-zenthar-text-primary)">{selectedStat.status}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="h-full bg-(--color-zenthar-carbon) rounded-2xl border border-brand-sage/10 flex flex-col items-center justify-center text-brand-sage gap-4">
                ...
                <p className="text-sm font-black text-(--color-zenthar-text-primary) uppercase tracking-widest">Select a STAT Request</p>
              </div>
            )}
          </AnimatePresence>
        </div>
