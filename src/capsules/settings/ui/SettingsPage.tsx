  ...
  return (
    <div className="h-full flex gap-6 p-4 bg-[#F8F9FA]">
      ...
      <aside className="w-72 flex flex-col gap-4">
        <div className="bg-white rounded-4xl border border-brand-sage/10 p-6 shadow-sm">
            ...
            {modules.map((m) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 group ${
                  activeModule === m.id
                    ? "bg-brand-deep text-white shadow-lg shadow-brand-deep/20 scale-[1.02]"
      ...
      {/* 2. MAIN CONTENT AREA */}
      <main className="flex-1 bg-white rounded-[2.5rem] border border-brand-sage/10 shadow-sm flex flex-col overflow-hidden relative">
        {/* HEADER */}
        <header className="px-10 py-8 border-b border-brand-sage/5 flex items-center justify-between bg-white/80 backdrop-blur-xl sticky top-0 z-20">
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 bg-brand-mist rounded-2xl flex items-center justify-center text-brand-primary shadow-inner">
          ...
          <button
            ...
            className="group flex items-center gap-3 pl-4 pr-6 py-3 bg-brand-deep text-white rounded-2xl text-xs font-bold hover:bg-brand-primary transition-all shadow-xl shadow-brand-deep/10"
          >
            ...
            <div className="max-w-6xl mx-auto space-y-3">
              {data.map((item, idx) => (
                <motion.div
                  ...
                  key={item.id || idx}
                  className="group bg-white border border-brand-sage/10 rounded-2xl p-4 flex items-center gap-6 hover:shadow-xl hover:shadow-brand-mist hover:border-brand-primary/20 transition-all duration-300"
                >
                  {/* Index / Status */}
