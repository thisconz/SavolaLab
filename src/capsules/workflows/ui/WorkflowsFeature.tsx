...
const formatDate = (d?: string) => d ? new Date(d).toLocaleString("en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "—";
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span className={clsx("text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-widest border", styles[status] ?? "bg-(--color-zenthar-void) border-brand-sage/20 text-brand-sage")}>
      {status}
    </span>
  );
};
...
export const WorkflowsFeature: React.FC = memo(() => {
  ...
  useEffect(() => {
    LabApi.getSamples().then(setSamples).finally(() => setLoading(false));
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      on("WORKFLOW_COMPLETED", (data) => {
        setLiveExIds((prev) => { const n = new Set(prev); n.delete(data.execution_id); return n; });
        refresh();
      }),
  ...
  const filteredSamples = useMemo(() => {
    ...
    return samples.filter((s) =>
      !q || [s.batch_id, s.source_stage, s.sample_type].some((v) => String(v ?? "").toLowerCase().includes(q))
    );
  }, [samples, searchQuery]);
          ...
          <div className="flex flex-col h-full bg-(--color-zenthar-carbon)/50">
            <div className="p-4 border-b border-brand-sage/10">
              <div className="relative">
                ...
                <input type="text" placeholder="Filter batch ID..."
            ...
            <div className="flex-1 overflow-auto custom-scrollbar p-3 space-y-2">
              {filteredSamples.map((sample) => (
                <button key={sample.id} onClick={() => setSelectedSample(sample)}
                  className={clsx("w-full text-left p-4 rounded-2xl transition-all border group",
                    selectedSample?.id === sample.id
                      ? "bg-brand-primary border-brand-primary text-white shadow-lg"
                      : "bg-(--color-zenthar-void) border-brand-sage/10 hover:border-brand-primary/30 text-white")}>
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-xs font-black font-mono tracking-wide">{sample.batch_id}</span>
                    <span className={clsx("text-[8px] px-1.5 py-0.5 rounded font-black uppercase",
                      selectedSample?.id === sample.id ? "bg-white/20" : "bg-(--color-zenthar-carbon)")}>
                      {sample.priority}
                    </span>
                  </div>
                  <div className={clsx("text-[9px] uppercase font-bold opacity-70",
        ...
        >
          {!selectedSample ? (
            <div className="h-full flex flex-col items-center justify-center text-brand-sage gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Select a sample to view executions</p>
            </div>
          ) : (
                ...
                <div className="flex gap-1 bg-(--color-zenthar-void) p-1 rounded-xl">
                  {["ALL", "IN_PROGRESS", "COMPLETED", "FAILED"].map((s) => (
                    <button key={s} onClick={() => setStatusFilter(s)}
                      className={clsx("px-3 py-1.5 rounded-lg text-[9px] font-black transition-all uppercase",
                        statusFilter === s ? "bg-brand-primary text-white shadow-sm" : "text-brand-sage hover:text-white")}>
                      {s}
                    </button>
                  ))}
                </div>
                <span className="text-[10px] font-mono text-brand-primary">{filteredExecutions.length} execution{filteredExecutions.length !== 1 ? "s" : ""}</span>
              </div>
                ...
                <AnimatePresence mode="popLayout">
                  {filteredExecutions.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-48 text-brand-sage gap-3">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest opacity-50">No executions found</p>
                    </div>
                  ) : (
                    filteredExecutions.map((execution, idx) => (
                      <motion.div key={execution.id}
                        ...
                        className={clsx(
                          ...
                          liveExIds.has(execution.id) ? "border-brand-primary/40 shadow-lg shadow-brand-primary/10" : "border-brand-sage/15"
                        )}
                      >
                        {/* Execution header */}
                        <div className="flex items-center justify-between mb-5">
                          <div className="flex items-center gap-4">
                            <div className={clsx("p-2.5 rounded-xl", execution.status === "FAILED" ? "bg-rose-500/10" : "bg-(--color-zenthar-carbon)")}>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-[9px] font-mono text-brand-sage/40">#{execution.id}</span>
                              </div>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-[8px] font-black text-brand-sage/50 uppercase">Cycle Time</p>
                            ...
                            <div key={step.id}
                              ...
                              <div className="flex items-center gap-3">
                                <div className={clsx("w-2 h-2 rounded-full",
                                  ...
                                  step.status === "FAILED"    ? "bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.4)]" :
                                  step.status === "IN_PROGRESS" ? "bg-brand-primary animate-pulse" :
                                  "bg-brand-sage/20")} />
                                <span className="text-[10px] font-bold text-white uppercase">{step.test_type}</span>
                                <span className="text-[8px] font-mono text-brand-sage/30">step {step.sequence_order}</span>
                              </div>
                              <div className="flex items-center gap-4 text-[10px] font-mono">
                                {step.result_value != null && (
                                  <span className="text-brand-primary font-bold">{step.result_value}</span>
                                )}
                                <span className="text-brand-sage/50">{getCycleTime(step.started_at, step.completed_at)}</span>
                              </div>
                            </div>
                          ))}
                        </div>
