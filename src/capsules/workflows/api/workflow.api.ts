...
export interface WorkflowStepPayload { status?: WorkflowStatus; test_id?: number; result_value?: number; }
...
export const WorkflowApi = {
  ...
  executeWorkflow: async (workflowId: number, sampleId: number): Promise<{ id: number }> => {
    const res = await api.post<any>(`/workflows/${workflowId}/execute`, { sample_id: sampleId });
