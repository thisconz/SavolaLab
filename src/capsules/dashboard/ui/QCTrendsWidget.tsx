...
interface Props { tests: TestResult[]; loading?: boolean; }

const CHART = {
  text:    { fontSize: 9, fontWeight: 700, fill: "#94a3b8", fontFamily: "inherit" },
  grid:    { stroke: "rgba(148,163,184,0.08)", strokeDasharray: "4 4" },
  tooltip: {
    contentStyle: { backgroundColor: "var(--color-zenthar-carbon)", borderRadius: "14px", border: "1px solid rgba(148,163,184,0.15)", padding: "10px 14px" },
    labelStyle: { fontSize: "9px", fontWeight: 900, textTransform: "uppercase" as const, color: "#94a3b8" },
  },
};
...
export const QCTrendsWidget: React.FC<Props> = memo(({ tests, loading = false }) => {
  const data = useMemo(() => {
    ...
    tests.forEach((t) => {
      ...
      const date = new Date(t.performed_at).toLocaleDateString(undefined, { month: "numeric", day: "numeric" });
    ...
    <div className="h-60 w-full">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 20, right: 8, left: -24, bottom: 0 }}>
          ...
          <Legend verticalAlign="top" align="right" wrapperStyle={{ fontSize: 9, fontWeight: 800, textTransform: "uppercase", paddingBottom: 12 }} />
          <Area type="monotone" dataKey="avg" name="Quality Index" stroke="var(--color-brand-primary)" strokeWidth={2.5}
            fill="url(#trendGrad)" dot={{ r: 3, fill: "var(--color-zenthar-carbon)", stroke: "var(--color-brand-primary)", strokeWidth: 2 }}
            activeDot={{ r: 5, strokeWidth: 0, fill: "var(--color-brand-primary)" }} animationDuration={1600} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
const TrendLoading = () => (
  <div className="h-60 w-full bg-(--color-zenthar-graphite)/50 animate-pulse rounded-2xl flex items-center justify-center">
    <div className="w-1/2 h-0.5 bg-brand-primary/10 rounded-full overflow-hidden">
      <motion.div animate={{ x: ["-100%", "100%"] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-1/3 h-full bg-brand-primary/30" />
    </div>
  </div>
);
...
const TrendEmpty = () => (
  <div className="h-60 w-full flex flex-col items-center justify-center border border-dashed border-brand-sage/20 rounded-2xl gap-2">
    <span className="text-[9px] font-black text-brand-sage/40 uppercase tracking-[0.3em]">Waiting for data...</span>
  </div>
);
