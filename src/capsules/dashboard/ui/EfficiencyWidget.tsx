...
const CustomYTick = (props: any) => {
  ...
  return (
    <text x={x - 8} y={y + 4} textAnchor="end" fontSize={9} fontWeight={700} fontFamily="inherit" fill="rgba(148,163,184,0.6)" style={{ textTransform: "uppercase" }}>
      {label}
    </text>
  );
};
...
export const EfficiencyWidget: React.FC<Props> = memo(({ samples, loading = false }) => {
  const data = useMemo(() => {
    ...
    return Object.entries(stageData)
      .map(([name, { sum, count }]) => ({ name, avgTests: parseFloat((sum / count).toFixed(1)), totalTests: sum }))
  ...
  return (
    <div className="h-60 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 8, right: 32, left: 0, bottom: 8 }} barSize={10}>
          ...
          <YAxis type="category" dataKey="name" tick={<CustomYTick />} axisLine={false} tickLine={false} width={84} />
          ...
          {/* Ghost track */}
          <Bar dataKey={() => maxVal * 1.1} fill="rgba(148,163,184,0.04)" radius={[0, 4, 4, 0]} isAnimationActive={false} />
          {/* Active bar */}
          <Bar dataKey="avgTests" radius={[0, 4, 4, 0]} animationDuration={1000}>
            {data.map((_, i) => (
              <Cell key={i} fill={i === 0 ? "var(--color-brand-primary)" : `rgba(14,165,233,${0.55 - i * 0.07})`} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
  ...
  <div className="h-60 w-full flex flex-col gap-4 py-4">
    {[...Array(5)].map((_, i) => (
      <div key={i} className="flex items-center gap-4">
        ...
        <div className="flex-1 h-3 bg-(--color-zenthar-graphite) rounded animate-pulse" style={{ width: `${95 - i * 12}%` }} />
      </div>
    ))}
  </div>
);
...
const EfficiencyEmpty = () => (
  <div className="h-60 w-full flex flex-col items-center justify-center border border-dashed border-brand-sage/20 rounded-2xl">
    <p className="text-[10px] font-black text-brand-sage uppercase tracking-widest">No efficiency data</p>
  </div>
);
