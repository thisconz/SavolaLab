...
export const PlantOverviewWidget: React.FC<Props> = memo(({ samples, loading = false }) => {
  const data = useMemo(() => {
  ...
  return (
    <div className="h-60 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 16, right: 8, left: -22, bottom: 0 }} barSize={32}>
          <CartesianGrid strokeDasharray="2 4" stroke="rgba(148,163,184,0.08)" vertical={false} />
          <XAxis dataKey="name" tick={<CustomXTick />} axisLine={false} tickLine={false} dy={8} />
          <YAxis tick={{ fontSize: 8, fontFamily: "inherit", fontWeight: 600, fill: "rgba(148,163,184,0.45)" }} axisLine={false} tickLine={false} domain={[0, maxVal + 2]} />
...
const PlantLoading = () => (
  <div className="h-60 w-full flex items-end justify-around pb-6 px-4 gap-2">
    {[...Array(7)].map((_, i) => (
      <div key={i} className="flex-1 bg-(--color-zenthar-graphite) rounded-t-lg animate-pulse" style={{ height: `${20 + i * 11}%`, opacity: 0.4 + i * 0.08 }} />
    ))}
  </div>
);
...
const PlantEmpty = () => (
  <div className="h-60 w-full flex flex-col items-center justify-center bg-(--color-zenthar-graphite)/50 rounded-2xl border border-dashed border-brand-sage/20">
    <p className="text-[10px] font-black text-brand-sage uppercase tracking-widest">No throughput data</p>
  </div>
);
