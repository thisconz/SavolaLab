...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from "@/src/lib/recharts";
  ...
  return (
    <div className="h-60 w-full relative">
      ...
      <div className="absolute top-2 right-2 flex gap-1 opacity-20">
        {[1,2,3].map(i => <div key={i} className="w-1 h-1 bg-brand-primary rounded-[1px]" />)}
      </div>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 28, right: 8, left: -28, bottom: 0 }} barSize={48}>
          ...
          <YAxis tick={{ fontSize: 8, fontFamily: "inherit", fontWeight: 600, fill: "rgba(148,163,184,0.5)" }} axisLine={false} tickLine={false} />
          <Tooltip {...TOOLTIP_STYLE} cursor={{ fill: "rgba(148,163,184,0.03)" }}
            formatter={(v: number, name: string) => [v, (PRIORITY_CONFIG[name as keyof typeof PRIORITY_CONFIG]?.label ?? name)]} />
          <Bar dataKey="value" radius={[6, 6, 0, 0]} animationDuration={1200}>
            {data.map((entry) => (
              <Cell key={entry.name} fill={PRIORITY_CONFIG[entry.name as keyof typeof PRIORITY_CONFIG]?.color ?? "#64748b"}
                style={entry.name === "STAT" ? { filter: "drop-shadow(0 0 8px rgba(239,68,68,0.4))" } : undefined} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
const PriorityLoading = () => (
  <div className="h-60 w-full flex items-end justify-center gap-8 pb-10">
    {[30, 60, 45].map((h, i) => (
      <div key={i} className="w-12 bg-(--color-zenthar-graphite) rounded-t animate-pulse" style={{ height: `${h}%`, opacity: 0.3 + i * 0.2 }} />
    ))}
  </div>
);
