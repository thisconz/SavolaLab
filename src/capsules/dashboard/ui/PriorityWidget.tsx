...
const CustomTick = (props: any) => {
  ...
  const config = PRIORITY_CONFIG[
    payload.value as keyof typeof PRIORITY_CONFIG
  ] ?? { color: "#94a3b8", label: payload.value };
  return (
    <text
      x={x}
      y={y + 14}
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(v: number, name: string) => [
                v,
                PRIORITY_CONFIG[name as keyof typeof PRIORITY_CONFIG]?.label ??
                  name,
              ]}
            />
            <Bar dataKey="value" radius={[6, 6, 0, 0]} animationDuration={1200}>
              {data.map((entry) => (
                <Cell
                  key={entry.name}
                  fill={
                    PRIORITY_CONFIG[entry.name as keyof typeof PRIORITY_CONFIG]
                      ?.color ?? "#64748b"
                  }
                  style={
                    entry.name === "STAT"
