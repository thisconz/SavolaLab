  ...
  const trends = useMemo(() => {
    ...
    const getCountInWindow = (
      ...
      return list.filter((item) => {
        ...
        return (
          time > now - daysStart * DAY_MS && time <= now - daysEnd * DAY_MS
        );
      }).length;
    };
    ...
    return {
      active: calculateTrend(
        getCountInWindow(active, 1, 0),
        getCountInWindow(active, 2, 1),
      ),
      ...
      stat: calculateTrend(
        getCountInWindow(stats, 1, 0),
        getCountInWindow(stats, 2, 1),
      ),
    };
  }, [samples, tests]);
    ...
    <div className="flex flex-col gap-6 h-full overflow-y-auto pb-8 pr-4 custom-scrollbar">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
        <MetricCard
          ...
          value={
            samples.filter((s) => !["COMPLETED", "ARCHIVED"].includes(s.status))
              .length
          }
          trend={trends.active}
          icon={FlaskConical}
            ...
            <div className="flex flex-col min-h-60">
              {loading ? (
                ...
                alerts.map((alert) => (
                  <div
                    key={alert.id}
                    className="flex items-start p-3 border-b border-brand-sage/5 hover:bg-brand-primary/5 transition-colors"
                  >
                    <div className="mt-0.5 mr-3 shrink-0 text-brand-primary">
                      {alert.type === "WORKFLOW_FAILURE" ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-bold text-brand-deep leading-tight truncate">
                        {alert.message}
                      </p>
  ...
  return (
    <div className="bg-white border border-brand-sage/10 rounded-2xl p-6 shadow-sm relative overflow-hidden group">
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div
          className={`p-2 rounded-lg ${(colorMap as any)[color] || colorMap["brand-primary"]}`}
        >
          <Icon className="w-5 h-5" />
        </div>
        <div
          className={`text-[10px] font-mono font-bold ${trend.startsWith("+") ? "text-emerald-500" : trend.startsWith("-") ? "text-red-500" : "text-brand-sage"}`}
        >
          {trend}
        </div>
      </div>
