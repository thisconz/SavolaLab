...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div
      className={`min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden font-sans transition-colors duration-700 ${error ? "bg-red-950/20" : "bg-(--color-zenthar-void)"}`}
    >
      ...
      <div
        className={`absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full blur-[150px] pointer-events-none transition-colors duration-1000 ${error ? "bg-red-500/10" : "bg-brand-sage/5"}`}
      />
      ...
      >
        <header className="text-center mb-10">
          ...
          <div className="flex items-center justify-center gap-3 mt-2">
            ...
            <span className="text-[10px] font-mono text-brand-sage tracking-[0.3em] uppercase opacity-60">
        ...
        <main className="bg-(--color-zenthar-carbon)/95 backdrop-blur-3xl rounded-[2.5rem] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.8)] overflow-hidden border border-brand-sage/10 relative">
              ...
              {isRegistering ? (
                ...
                >
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      <h2 className="text-[11px] font-black text-(--color-zenthar-text-primary) uppercase tracking-widest">
                        Select Personnel
                      </h2>
                      <p className="text-[9px] text-brand-sage font-mono uppercase mt-1">
                        Active Sync: {users.length} Users
                      </p>
                    </div>
                    <button
                      onClick={fetchUsers}
                      className="p-2 hover:bg-(--color-zenthar-graphite) rounded-lg transition-colors group"
                    >
                      <RefreshCw
                        className={`w-3.5 h-3.5 text-brand-sage group-hover:text-brand-primary ${loading && "animate-spin"}`}
                      />
                    </button>
                  </div>
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="relative">
                              ...
                              {user.status === "online" && (
                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-500 border-2 border-(--color-zenthar-carbon) rounded-full" />
                              )}
                            </div>
                            <div>
                  ...
                  <div className="p-5 bg-(--color-zenthar-void) rounded-2xl border border-brand-sage/10 shadow-2xl relative overflow-hidden">
                    <div className="flex items-center gap-4 relative z-10">
                      ...
                      <div className="flex-1">
                        <p className="text-[8px] font-mono text-brand-primary uppercase tracking-[0.2em] mb-1">
                          Personnel Verified
                        </p>
                        <h3 className="text-sm font-black text-(--color-zenthar-text-primary) uppercase tracking-wider leading-none">
                          {selectedUser.name}
                        </h3>
                        <p className="text-[9px] text-brand-sage font-mono uppercase tracking-tighter mt-1.5">
                          {renderedRole}
                        </p>
                      </div>
                      <button
                    ...
                    <div className="absolute top-0 right-0 p-2 opacity-10">
                      <Fingerprint className="w-12 h-12 text-(--color-zenthar-text-primary)" />
                    </div>
                  </div>
                      ...
                      {(["pin", "password"] as const).map((mode) => (
                        <button
                          ...
                          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                            authMode === mode
                              ? "bg-(--color-zenthar-carbon)] text-brand-primary shadow-lg border border-brand-sage/10"
                    ...
                    <div className="relative space-y-4">
                      <div className="relative group">
                        <input
                          ...
                          className={`w-full bg-(--color-zenthar-graphite)/30 border-2 rounded-2xl py-6 px-6 text-center text-3xl font-mono tracking-[0.6em] focus:outline-none transition-all ${
                            error
                              ? "border-red-500/50 bg-red-500/5 text-red-500 ring-4 ring-red-500/10"
                      ...
                      {error && (
                        <motion.div
                          ...
                          className="flex items-center justify-center gap-2 text-red-500 bg-red-500/10 backdrop-blur-sm py-3 px-4 rounded-xl border border-red-500/20 shadow-sm"
                        >
          ...
          <footer className="px-10 py-5 bg-(--color-zenthar-graphite)/30 border-t border-brand-sage/5 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[9px] text-brand-sage font-mono font-bold uppercase tracking-[0.2em]">
                Network_Stable
              </span>
            </div>
            <div className="h-4 w-px bg-brand-sage/20" />
            <p className="text-[9px] text-brand-sage/60 font-mono font-bold uppercase tracking-widest">
